                ...
                if ai_classification:
                    ...
                    for col in dataset.columns:
                        if col.name == column_name:
                            col.sensitivity_type = combined.sensitivity_type.value
                            col.category = combined.category.value
...
{{
  ...
  "justification": "Explication courte en français"
    ...
    def _combine_classifications(
        ...
        if ai_based.confidence >= 85:
            return ColumnClassification(
                column_name=rule_based.column_name,
                sensitivity_type=ai_based.sensitivity_type,
                category=ai_based.category,
                confidence=min(ai_based.confidence + 5, 100),
                justification=f"{ai_based.justification} (validé par IA haute-fiance)",
            )

        # Both agree -> boost confidence
        if rule_based.sensitivity_type == ai_based.sensitivity_type:
             # Agree on sensitivity type, use best category
            chosen_cat = ai_based.category if ai_based.confidence >= rule_based.confidence else rule_based.category
            combined_confidence = min(max(rule_based.confidence, ai_based.confidence) + 10, 100)
            ...
            return ColumnClassification(
                ...
                justification=f"{rule_based.justification} (confirmé par IA)",
            )
        ...
        if sensitivity_rank.get(ai_based.sensitivity_type, 0) > sensitivity_rank.get(rule_based.sensitivity_type, 0) and ai_based.confidence > 60:
            return ColumnClassification(
                column_name=rule_based.column_name,
                sensitivity_type=ai_based.sensitivity_type,
                category=ai_based.category,
                confidence=ai_based.confidence,
                justification=f"{ai_based.justification} (Détecté par IA, manqué par règles)",
            )
        
        # Default to weighted average preferring higher confidence
        if rule_based.confidence > ai_based.confidence:
            chosen_type = rule_based.sensitivity_type
            chosen_cat = rule_based.category
            justif = f"{rule_based.justification} (IA suggérait {ai_based.sensitivity_type})"
        else:
            chosen_type = ai_based.sensitivity_type
            chosen_cat = ai_based.category
            justif = f"{ai_based.justification} (Règles suggéraient {rule_based.sensitivity_type})"

        return ColumnClassification(
            column_name=rule_based.column_name,
            sensitivity_type=chosen_type,
            category=chosen_cat,
            confidence=max(rule_based.confidence, ai_based.confidence),
