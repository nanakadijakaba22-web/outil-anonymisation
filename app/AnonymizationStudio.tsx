...
export default function AnonymizationStudio() {
  ...
  // corriger (cohérent avec le reste du fichier)
const [anonymized, setAnonymized] = React.useState<Record<string, any>[]>([]);

const [compliance, setCompliance] = React.useState({
  individualisation: 0, // en %
  correlation: 0,       // en %
  inference: 0          // en %
});
  ...
  (Array.isArray(rowsPreview) && rowsPreview.length > 0) ||
      ...
      Papa.parse<Record<string, any>>(file, {
        ...
        complete: async (res) => {
          try {
            ...
            console.log(`Anonymisation effectuée : ${kept.length}/${rows.length} lignes prêtes au téléchargement.`);

            evaluateCompliance(kept, map.qids, map.sensitive, k);
            /** Recalcule les 3 scores Loi 25 sur le dataset anonymisé */
function evaluateCompliance(
  ...
  const indiv = computeIndividualisation(data, qids); // %
  const corr  = computeCorrelation(data, qids, k);   // %
  const infer = computeInference(data, qids, sensitiveCol); // %

  // -> ici, mets à jour ce que ta UI attend (remplace setCompliance... par tes setters)
  setCompliance({
  individualisation: indiv,
  correlation: corr,
  inference: infer,
...
/** % de lignes uniques selon les QI (1 = unique) */
function computeIndividualisation(data: Record<string, any>[], qids: string[]): number {
  if (!data.length || !qids.length) return 0;
  const key = (r: any) => qids.map((q) => String(r[q] ?? "")).join("|");
  const counts = new Map<string, number>();
  for (const r of data) counts.set(key(r), (counts.get(key(r)) ?? 0) + 1);
  let uniques = 0;
  for (const [, c] of counts) if (c === 1) uniques += 1;
  return (uniques / data.length) * 100;
}

/** % de lignes appartenant à des groupes < k (risque de corrélation / ré-identification) */
function computeCorrelation(data: Record<string, any>[], qids: string[], k: number): number {
  if (!data.length || !qids.length) return 0;
  const key = (r: any) => qids.map((q) => String(r[q] ?? "")).join("|");
  const counts = new Map<string, number>();
  for (const r of data) counts.set(key(r), (counts.get(key(r)) ?? 0) + 1);
  let atRisk = 0;
  for (const r of data) {
    const c = counts.get(key(r)) ?? 0;
    if (c < k) atRisk += 1;
  }
  return (atRisk / data.length) * 100;
}

/** % d’inférence : moyenne, par groupe QI, de la “prévisibilité” du sensible */
function computeInference(
  data: Record<string, any>[],
  qids: string[],
  sensitiveCol?: string | null
): number {
  if (!data.length || !qids.length || !sensitiveCol) return 0;
  const key = (r: any) => qids.map((q) => String(r[q] ?? "")).join("|");
  ...
  // pour chaque groupe: si 1 seule valeur sensible -> 100% pour ce groupe, sinon <100
  let total = 0;
  let weighted = 0;
  for (const [, rows] of groups) {
    const distinct = new Set(rows.map((x) => String(x[sensitiveCol!] ?? "")));
    const grpRisk = distinct.size <= 1 ? 100 : (1 - 1 / distinct.size) * 100;
    weighted += grpRisk * rows.length;
    total += rows.length;
  }
...
function downloadAnonymized() {
  if (!Array.isArray(anonymized) || anonymized.length === 0) {
    alert("Aucune donnée anonymisée prête. Clique d’abord sur Exécuter.");
    return;
  }
  const cols = (Array.isArray(columns) && columns.length > 0)
    ? columns
    : Object.keys(anonymized[0] ?? {});

  const csv = Papa.unparse(anonymized, { columns: cols });
  const blob = new Blob([csv], { type: "text/csv;charset=utf-8" });
  const fname = `donnees_anonymisees_${Date.now()}.csv`;
  saveAs(blob, fname);
}
...
<div className="flex flex-wrap items-center gap-2">
  ...
  <Button
    variant="outline"
    className="gap-2"
    onClick={() => runProfiler(rowsFromPreview())}
    disabled={!hasData}
  ...
  <Button
    variant="outline"
    className="gap-2"
    onClick={() => runRecommendations(rowsFromPreview())}
    disabled={!hasData}
  ...
  variant="outline"
  className="gap-2"
  onClick={() => runPipelines(rowsFromPreview())}   // <-- on enveloppe l’appel avec les données
  disabled={!hasData}
...
>
  ...
  variant="outline"
  className="gap-2"
  onClick={() => runAutoAnonymisation(rowsFromPreview())}      // <-- pas de paramètre ici
  disabled={!hasData}
...
>
  ...
  <div className="ml-auto flex items-center gap-2">
    ...
    <Button
      variant="outline"
      className="gap-2"
      onClick={() => runAudit(rowsFromPreview(), kMin)}
      disabled={!hasData}
...
<Button
  className="gap-2"
  onClick={() => executeAnonymization()}   // ✔ lance l’anonymisation (sans télécharger)
  disabled={!hasData}                      // ✔ désactive si pas de données
  title={!hasData ? "Importer des données avant d’exécuter" : ""}
>
  Exécuter
</Button>

{/* 8) Télécharger (exporter le dernier résultat anonymisé) */}
<Button
  className="gap-2"
  onClick={() => downloadAnonymized()}     // ✔ exporte depuis l’état `anonymized`
  disabled={!(Array.isArray(anonymized) && anonymized.length > 0)}
  title={!(Array.isArray(anonymized) && anonymized.length > 0)
    ? "Clique d’abord sur Exécuter pour préparer les données"
    : ""}
>
  Télécharger
</Button>
